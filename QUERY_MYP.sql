create or alter procedure QUERY_MYP (
    DATE_BEGIN date,
    DATE_END date,
    N_GLC_GROUP integer,
    N_PODR integer)
...
(SELECT res FROM recounttorur((select RESULT from  LT_REWARD_P(p.n, cast(:date_begin as timestamp),cast(:date_end as timestamp))), p.n_curr2, p.icept_date)) AS COST_REWARD,
curr.one_c_code as CURR,
SUM(pp.brok_rate*pp.payment_amount/120) as TOTAL_REV_ORIG_CUR_NET_VAT,
(select RESULT from  LT_REWARD_P(p.n, cast(:date_begin as timestamp),cast(:date_end as timestamp))) as COST_REWARD_ORIG_CUR,
0 as DFRD_REV_ON_INCEPT,
(select RESULT from  LT_REWARD_P(p.n, cast(:date_begin as timestamp),cast(:date_end as timestamp))) as PER_AMRTZD_REV,
p.n_curr2 as N_CURR,
podr.n as PODR_N,
...
left join curr on curr.n = p.n_curr2

WHERE

p.posting_date<=:date_end AND
p.posting_date is not null AND
p.n_fee_comm IN (2,3)  AND
p.exp_date-p.icept_date>730   and
p.exp_date+30-730>=:DATE_BEGIN and
/*p.icept_date+30<=:DATE_BEGIN and*/
--(p.is_rejected='F' or p.is_rejected is null or  p.date_reject>:date_end) and
...
PRODUCT, N_SOURCE, IS_NEW, p.is_recurring, DEP, ng.NOM_GR_ONE_C_CODE, EFFECTIVE_DATE, EXPIRY_DATE,
p.n_curr2, p.icept_date, CURR , N_CURR, PODR_N, REF_CERT
...
(SELECT res FROM recounttorur((select RESULT from  LT_REWARD_SA(sa.n, cast(:date_begin as timestamp), cast(:date_end as timestamp))), sa.n_curr, sa.date_beg)) AS COST_REWARD,
curr.one_c_code as CURR,
SUM(fp.payment_amount/1.20) AS TOTAL_REV_ORIG_CUR_NET_VAT,
(select RESULT from  LT_REWARD_SA(sa.n, cast(:date_begin as timestamp), cast(:date_end as timestamp)))  as COST_REWARD_ORIG_CUR,
0 as DFRD_REV_ON_INCEPT,
(select RESULT from  LT_REWARD_SA(sa.n, cast(:date_begin as timestamp), cast(:date_end as timestamp))) as PER_AMRTZD_REV,
curr.n as N_CURR,
podr.n as PODR_N,
...
sa.date_end+30-730>=:DATE_BEGIN and
/*sa.date_beg+30<=:DATE_BEGIN and*/
...
group by
STP_MYP, CLIENT_GROUP, REWARD_BEG, REWARD_END, Reward_Type, N_PAYER, ACC_EXECUTIVE, STATUS, LOB,
PRODUCT, N_SOURCE, IS_NEW, sa.is_recurring, DEP, ng.NOM_GR_ONE_C_CODE, EFFECTIVE_DATE, EXPIRY_DATE,
sa.n_curr, sa.date_beg, CURR,  N_CURR, PODR_N, REF_CERT
...
(SELECT res FROM recounttorur((select RESULT from  LT_REWARD_CN(cn.n, cast(:date_begin as timestamp), cast(:date_end as timestamp))), a.n_curr_comis, cn.date_beg)) AS COST_REWARD,
curr.one_c_code as CURR,
SUM(a.cost_bonus*lcr.perc_reins*lcr.perc_comis_aonRus*payer.perc_cedent /1200000) AS TOTAL_REV_ORIG_CUR_NET_VAT,
(select RESULT from  LT_REWARD_CN(cn.n, cast(:date_begin as timestamp), cast(:date_end as timestamp))) AS COST_REWARD_ORIG_CUR,
0 as DFRD_REV_ON_INCEPT,
(select RESULT from  LT_REWARD_CN(cn.n, cast(:date_begin as timestamp), cast(:date_end as timestamp))) as PER_AMRTZD_REV,
curr.n as N_CURR,
podr.n as PODR_N,
...
cn.date_end+30-730>=:DATE_BEGIN AND
/*cn.date_beg+30<=:DATE_BEGIN       AND*/
...
CLIENT_GROUP, REWARD_BEG, REWARD_END, N_PAYER, ACC_EXECUTIVE, STATUS, rl.n_ri_contract, LOB, PRODUCT, N_SOURCE,
IS_NEW, cn.is_recurring, DEP, ng.NOM_GR_ONE_C_CODE, EFFECTIVE_DATE, EXPIRY_DATE, a.n_curr_comis, COST_REWARD,
CURR, COST_REWARD_ORIG_CUR, N_CURR, PODR_N, REF_CERT
